...
logger = logging.getLogger("acquisition.collectors.nifty50")


class Nifty50EquityCollector(BaseCollectorPlugin):
    """
    Sprint D2 Production NIFTY 50 Equity Stock OHLCV Collector Plugin.
    Connects to production UpstoxApiClient for real historical OHLCV data.
    """
    ...
    def source_name(self) -> str:
        return "NIFTY50_EQUITY_COLLECTOR"
    ...
    def fetch_historical_chunk(self, symbol: str, start_date: str, end_date: str) -> Optional[pa.Table]:
        """
        Fetches production equity OHLCV candles using UpstoxApiClient.
        ...
        for c in raw_candles:
            ...
            vwaps.append(float(c[4]))  # Close price fallback for VWAP
...
DataSourceRegistry.register_collector(Nifty50EquityCollector)
