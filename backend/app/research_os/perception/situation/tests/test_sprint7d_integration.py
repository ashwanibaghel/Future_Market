...
def test_sprint7d_full_cognition_pipeline():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Step 2: Initialize PerceptionEngine with all 3 Perception Modules (7B, 7C, 7D)
        # Register in RANDOM order to test Kahn's Topological Sorting!
        perception_engine = PerceptionEngine()
        situation_module = SituationAssessmentModule()
        regime_module = MarketRegimeModule()
        pattern_module = PatternRecognitionModule()

        perception_engine.register_module(situation_module)
        perception_engine.register_module(regime_module)
        perception_engine.register_module(pattern_module)

        # Verify Topological Resolution Order: Pattern -> Regime -> Situation
        sorted_names = [m.module_name for m in perception_engine._sorted_modules]
        assert sorted_names == ["pattern_recognition", "market_regime", "situation_assessment"]
        ...
        session_id = "SESS-SITUATION-E2E-001"
        ...
        # Verify Situation Assessment Perception Output
        sit_perception = first_frame.get_perception("situation_assessment")
        assert sit_perception is not None
        assert sit_perception["confidence"] > 0.0
        meta = sit_perception["metadata"]
        assert "macro_situation_label" in meta
        assert "risk_environment_label" in meta
        assert "conflict_status" in meta
...
if __name__ == "__main__":
    test_sprint7d_full_cognition_pipeline()
    print("\nSPRINT 7D INTEGRATION TEST PASSED SUCCESSFULLY!")
